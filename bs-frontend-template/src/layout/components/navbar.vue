<template>
  ...
  <div v-else class="flex items-center px-4 flex-wrap h-12 flex-1">
    <div class="layout-sidebar-logo flex relative shadow-lg w-60 leading-12 items-center">
  ...
  <div class="flex items-center flex-row-reverse px-4 min-width-32">
    ...
    <el-dropdown>
      <span class="el-dropdown-link flex flex-center px-2">
        ...
        />-->
        <span class="ml-2">{{ loginRequired ? userInfo.name : "Option" }}</span>
        <i class="el-icon-arrow-down el-icon--right" />
      </span>
      <template #dropdown>
        <el-dropdown-menu>
          <el-dropdown-item>
            <router-link to="/Personal">Account center</router-link>
          </el-dropdown-item>
          <el-dropdown-item divided>
            <el-link target="_blank" :underline="false" href="http://127.0.0.1:8000/admin">Go to Admin</el-link>
          </el-dropdown-item>
          <el-dropdown-item divided @click="logout" v-if="loginRequired">Logout</el-dropdown-item>
        </el-dropdown-menu>
      </template>
    </el-dropdown>
